    ...
    if request.method == "POST":
        ...
        for emp in employees:   
            ...
            if emp.empid == empID and flag :
                ...
                elif emp.position == "sales manager" :
                    messages.success(request,"Sales manager login sucessfully")
                    return  redirect("salesapp-sales")
                
                elif emp.position == "finance manager":
                    messages.success(request,"Finance manager login sucessfully")
                    return  redirect("dashboard-indexSVD")

                elif emp.position == "supplier manager ":
                    messages.success(request,"supplier manager login sucessfully")
                    return  redirect("supplierapp-index")

                elif emp.position == "stock manager":
                    messages.success(request,"stock manager login sucessfully")
                    return  redirect("stockapp-index")

                elif emp.position == "order manager ":
                    messages.success(request,"order manager login sucessfully")
    ...
    if 'username' not in request.session:
        ...
        if request.method == "POST":
            ...
            if len(otHours) == 0 :
                ...
                send_mail(
                        subject,message,'jayanandanafachion@gmail.com',['sasith1023@gmail.com']
                    )
